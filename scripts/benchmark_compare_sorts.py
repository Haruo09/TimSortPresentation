...
OUTPUT_IMAGE_PATH = "docs/timsort_vs_quicksort.png"
...
def main():
    ...
    fig.suptitle("Performance Comparison: TimSort vs  QuickSort (C Code)", fontsize=15)
    ...
    for i, (title, dist_gen) in enumerate(dists):
        ...
        qs_times = benchmark_c_func(c_lib.quickSortAlgo, sizes, dist_gen)

        axes[i].plot(sizes, ts_times, label="TimSort", color="crimson", linewidth=2)
        axes[i].plot(sizes, qs_times, label="QuickSort", color="royalblue", linewidth=2, linestyle="--")
