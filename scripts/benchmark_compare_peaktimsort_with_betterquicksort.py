...
OUTPUT_IMAGE_PATH = "docs/peaktimsort_vs_betterquicksort.png"
...
def main():
    ...
    fig.suptitle("Performance Comparison: Peak TimSort vs Better QuickSort (C Code)", fontsize=15)

    for i, (title, dist_gen) in enumerate(dists):
        print(f" -> Benchmarking: {title}...")
        ts_times = benchmark_c_func(c_lib.peakTimSortAlgo, sizes, dist_gen)
        qs_times = benchmark_c_func(c_lib.betterQuickSortAlgo, sizes, dist_gen)

        axes[i].plot(sizes, ts_times, label="PeakTimSort", color="crimson", linewidth=2)
